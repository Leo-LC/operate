...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const result = await supabase.from("recurring_cost_overrides").upsert(
  ...
  const { data: rule } = await supabase
    .from("recurring_costs").select("custom_allocations")
    .eq("id", params.id).eq("organization_id", DEFAULT_ORG_ID).maybeSingle();
  await supabase.from("recurring_costs").update({
  ...
  await supabase.from("finance_audit_events").insert({
    ...
    entity_type: "recurring_cost", entity_id: params.id, reason, payload: { year, month, amount },
  });
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  const result = await supabase.from("recurring_cost_overrides").delete()
    .eq("organization_id", DEFAULT_ORG_ID).eq("cost_rule_id", params.id)
    .eq("service_from", from).eq("service_to", to);
  if (result.error) return Response.json({ error: result.error.message }, { status: 500 });

  const { data: remaining, error: remainingError } = await supabase
    .from("recurring_cost_overrides").select("id").eq("organization_id", DEFAULT_ORG_ID).eq("cost_rule_id", params.id).limit(1);
  ...
  await supabase.from("finance_audit_events").insert({
    ...
    entity_type: "recurring_cost", entity_id: params.id, reason, payload: { year, month },
  });
