...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  // Lock identity: label/category cannot be changed after creation
  if ("label" in body || "category" in body) {
    return Response.json({ error: "Category and name cannot be changed after creation" }, { status: 400 });
  }
