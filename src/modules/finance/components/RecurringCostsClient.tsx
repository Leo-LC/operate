...
export function RecurringCostsClient() {
  ...
  const load = useCallback(async () => {
    ...
    setLocations(json.locations); setCosts(locationId ? json.costs : []); setSalaries(json.salaries ?? {}); setCanManage(json.canManage);
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12 }}>
      <Card onClick={() => router.push("/employees")} style={{ gap: 8, cursor: "pointer", transition: "border-color 150ms" }} className="hover:!border-[var(--line-strong)]"><span style={{ fontSize: 12, color: "var(--fg-4)" }}>Payroll</span><strong style={{ fontSize: 16 }}>Salary</strong><span className="mono" style={{ fontSize: 22 }}>฿{Number(salaries[locationId] ?? 0).toLocaleString()}</span><span style={{ fontSize: 12, color: "var(--fg-4)" }}>Click to edit salaries</span></Card>
